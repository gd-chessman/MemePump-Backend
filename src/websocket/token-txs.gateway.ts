    ...
    private handleBirdeyeMessage(message: any) {
        try {
            if (message.type === 'TXS_DATA') {
                const { tokenAddress, blockUnixTime, owner, source, txHash, side, volumeUSD, from, to } = message.data;
                
                if (tokenAddress && this.tokenSubscriptions.has(tokenAddress)) {
                    const subscribers = this.tokenSubscriptions.get(tokenAddress)!;
                    subscribers.forEach(clientId => {
                        this.server.to(clientId).emit('transaction', {
                            tokenAddress,
                            blockUnixTime,
                            owner,
                            source,
